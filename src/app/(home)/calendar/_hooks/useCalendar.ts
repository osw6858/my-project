...
export function useCalendar(events?: Event[]) {
  ...
  const getEventsForDate = (date: dayjs.Dayjs) => {
    return events?.filter((event) =>
      date.isBetween(dayjs(event.startDate).startOf('day'), dayjs(event.endDate).startOf('day'), "day", "[]")
    ) || [];
  };
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < startDay; i++) {
      const date = currentMonth.subtract(1, "month").date(prevMonthLastDay - (startDay - 1 - i));
